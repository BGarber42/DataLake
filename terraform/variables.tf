...
variable "s3_glacier_days" {
  description = "Number of days before moving S3 objects to Glacier (must be > s3_lifecycle_days)"
  type        = number
  default     = 90

  validation {
    condition     = var.s3_glacier_days >= 31
    error_message = "Glacier transition days must be at least 31."
  }
}
